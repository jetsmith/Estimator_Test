    ...
    with slim.arg_scope([slim.batch_norm, slim.dropout],
      ...
      with tf.variable_scope('Logits'):
        if global_pool:
          ...
          print('avgpool shape: ', net.get_shape())
        if not num_classes:
          return net, end_points
        # 1 x 1 x 1024
        logits = {}
        attribute=['jc', 'pc', 'pt', 'age', 'gender']
        num_classes=num_classes.split(',')
        for i, attr in enumerate(attribute):
            tmp_net = slim.dropout(net, keep_prob=dropout_keep_prob, scope=attr+'/Dropout_1b')
            logit = slim.conv2d(tmp_net, int(num_classes[i]), [1, 1], activation_fn=None,
                                 normalizer_fn=None, scope=attr+'/Conv2d_1c_1x1')
            #logits = slim.conv2d(net, num_classes, [1, 1], activation_fn=None,
            #                     normalizer_fn=slim.batch_norm, scope='Conv2d_1c_1x1')
            if spatial_squeeze:
              logit = tf.squeeze(logit, [1, 2], name=attr+'/SpatialSqueeze')
            logits[attr] = logit
            end_points[attr] = logit
        #if prediction_fn:
