...
def focal_loss(logits,
    ...
    Args:
     logits: A float tensor of shape [-1, num_classes] representing
        the predicted logits for each class
     labels: A float tensor of shape [-1] representing classification labels
     weights: A float tensor of shape [-1]
    ...
    # one-hot for labels
    cshape = tfe.get_shape(logits, 2)
    num_classes = cshape[1]
    target_tensor = tf.cast(tf.one_hot(labels, num_classes, 1, 0), sigmoid_p.dtype)
