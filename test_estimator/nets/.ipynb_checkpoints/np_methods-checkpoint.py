...
def bboxes_nms1(classes, scores, bboxes, nms_threshold=0.45, nms_threshold1=0.7):
    """Apply non-maximum selection to bounding boxes.
    """
    keep_bboxes = np.ones(scores.shape, dtype=np.bool)
    for i in range(scores.size-1):
        if keep_bboxes[i]:
            ...
            # Overlap threshold1 for non-keeping part of the different class
            not_keep_overlap = np.logical_and(overlap > nms_threshold1,
                    classes[(i+1):] != classes[i])
            keep_bboxes[(i+1):] = np.logical_and(
                    keep_bboxes[(i+1):],
                    np.logical_not(not_keep_overlap))
