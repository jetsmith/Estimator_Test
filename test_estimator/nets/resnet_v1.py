  ...
  with tf.variable_scope(scope, 'resnet_v1', [inputs], reuse=reuse) as sc:
    ...
    with slim.arg_scope([slim.conv2d, bottleneck,
      ...
      with (slim.arg_scope([slim.batch_norm], is_training=is_training)
        ...
        #    net = tf.reduce_mean(net, [1, 2], name='pool5', keep_dims=True)
        #    end_points['global_pool'] = net
        #for i, attr in enumerate(['age', 'gender']):
        #    if num_classes:
        #        print('net shape: ', net.get_shape())
        #        tmp_net = slim.conv2d(net, int(num_classes[i]), [1, 1], activation_fn=None,
        #                        normalizer_fn=None, scope=attr + '/logits')
        #        end_points[sc.name + attr +'/logits'] = tmp_net
        #    if spatial_squeeze:
        #        tmp_net = tf.squeeze(tmp_net, [1, 2], name=attr + '/SpatialSqueeze')
        #        end_points[sc.name + attr + '/spatial_squeeze'] = tmp_net
        #        logits[attr] = tmp_net
        #      #end_points['predictions'] = slim.softmax(net, scope='predictions')
